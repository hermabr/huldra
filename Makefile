...
dashboard-dev-backend:
	@bash -c 'set -euo pipefail; \
		...
		FURU_PATH="$$data_dir" FURU_RECORD_GIT=ignore uv run python e2e/generate_data.py --data-dir "$$data_dir"; \
		echo "Starting dashboard API with FURU_PATH=$$data_dir"; \
		FURU_PATH="$$data_dir" FURU_RECORD_GIT=ignore uv run uvicorn furu.dashboard.main:app --reload --host 0.0.0.0 --port 8000'
