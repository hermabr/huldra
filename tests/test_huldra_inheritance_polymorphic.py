"""Tests for Huldra inheritance with polymorphic dependencies.

Tests the pattern where:
- Data is an abstract base class (subclass of Huldra)
...
def test_train_with_data_a(huldra_tmp_root) -> None:
...
def test_train_with_data_b(huldra_tmp_root) -> None:
...
def test_same_train_different_data_produces_different_hashes(huldra_tmp_root) -> None:
    """Train with DataA vs DataB should have different huldra_hash."""
    ...
    hash_a = HuldraSerializer.compute_hash(train_a)
    hash_b = HuldraSerializer.compute_hash(train_b)
...
def test_data_subclasses_have_different_hashes(huldra_tmp_root) -> None:
    """DataA and DataB with same name should have different hashes."""
    data_a = DataA(name="same_name")
    data_b = DataB(name="same_name")

    hash_a = HuldraSerializer.compute_hash(data_a)
    hash_b = HuldraSerializer.compute_hash(data_b)
    assert hash_a != hash_b


def test_train_caches_correctly_with_polymorphic_data(huldra_tmp_root) -> None:
...
def test_abstract_data_base_class_raises_not_implemented(huldra_tmp_root) -> None:
